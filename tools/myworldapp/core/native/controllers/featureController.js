...
export class FeatureController extends BaseController {
     ...
     * @param  {object}   [individualChanges]
     * @return {string[]}     List with urns of updated features {updated_features}
     */
    async bulkUpdate(features, properties, individualChanges) {
        ...
        return this.view.runInTransaction(async () => {
            ...
            // loop through individualChanges and update each feature
            if (individualChanges) {
                for (const [urn, properties] of Object.entries(individualChanges)) {
                    const [featureType, pkey] = urn.split('/');
                    const table = this.view.table(featureType);
                    await table.updateManyFrom([pkey], properties);
                }
            }
