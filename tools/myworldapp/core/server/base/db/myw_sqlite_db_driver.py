...
class MywSqliteDbDriver(MywDbDriver):
    ...
    def sqlForTimestamp(self, timestamp):
        ...
        Due to SQLite timestamps requiring a different format, we return a formatted string instead
        """
    ...
    def sqlDefaultFor(self, column_desc, quote_string_values=True):
        ...
        if (myw_type == "timestamp") and (generator == "now_utc"):
            return "(strftime('%Y-%m-%dT%H:%M:%S', 'now'))"
        else:
    ...
    def addColumnSqls(self, schema, table_name, column_desc):
        ...
        if column_desc.isGeometry():
            ...
            )  # ENH: We could .. but it's hard

        # Generate the column def to continue validating
        sql_def = self.sqlColumnDefFor(column_desc)

        if column_desc.type == "timestamp" and "DEFAULT" in sql_def:
            raise MywCannotMutateException(
                "Cannot add date column with a default in-place"
            )  # SQLite syntax for doing this is radically different to the default case.
    ...
    def _createTriggerSqls(
        ...
        # If there's no trigger body, create a valid no operation trigger for sqlite
        if not trigger_body:
            trigger_body = "/* iqgeo_no_op */\n" "select 1 where 0;\n"
    ...
    def prepareForInsert(self, feature_type, rec):
        ...
        Allocates ID from shard (if neccessary)"""
    ...
    def _execute2(self, statement, params):
        ...
        self.progress(10, self, "Running sqlcipher3 isolation_level=None SQL:", statement)

        conn = sqlcipher3.connect(db_file, isolation_level=None)
