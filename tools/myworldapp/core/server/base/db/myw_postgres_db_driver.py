...
from myworldapp.core.server.base.db.myw_db_meta import MywDbTable, MywDbColumn
...
class MywPostgresDbDriver(MywDbDriver):
    ...
    def sequenceRangeSql(self, schema, table, field, min_value, max_value, restart=False):
        """
        Generate SQL to set the range for generator of TABLE.FIELD

        If RESTART is True, the SQL also sets the next value to MIN_VALUE"""
    ...
    def setSequenceRange(self, schema, table, field, min_value, max_value, restart=False):
        ...
        self.execute(self.sequenceRangeSql(schema, table, field, min_value, max_value, restart))
    ...
    def removeGeographyIndex(self, schema, table_name, column_name):
        """
        Remove a geography-cast index on the column, in preparation for conversion from geometry to
        geography.
        (note, the column name is not actually required, but we leave it as part of the interface
        to match add.)
        """

        db_index_name = self.dbIndexNameFor(schema, table_name, ["geography"], full=True)

        sql = f"DROP INDEX IF EXISTS {db_index_name};"
        self.execute(sql)
    ...
    def sqlTypeFor(self, field_name, type_desc):
        ...
        elif type_desc.base == "json":
            return "json"
        elif type_desc.base in ["image", "file"]:
    ...
    def sqlDefaultFor(self, column_desc, quote_string_values=True):
        ...
        if generator == "sequence":
            ...
            return "now()"
        elif (myw_type == "timestamp") and (generator == "now_utc"):
    ...
    def alterColumnSqls(
        ...
        # These Columns may not have their table set up, ensure it is so that we can generate
        # sequence names. (Note: it is slightly hacky to instantiate them without a table.)
        if old_column_desc.table is None:
            MywDbTable(schema, table_name, old_column_desc)

        if new_column_desc.table is None:
        ...
        if (
            old_column_desc.type_desc.base == "integer"
            and new_column_desc.type_desc.base == "bigint"
            and old_column_desc.generator == "sequence"
            and new_column_desc.generator == "sequence"
        ):
            # Sometimes sequencies migrated from older versions of PostgreSQL will have an upper
            # limit on their sequence that corresponds to 32bit int max rather than 64. If we are
            # bumping an integer field up to 64-bit, force the sequence up as well.
            sqls.append(
                self.sequenceRangeSql(
                    schema, table_name, old_column_desc.name, 1, 9223372036854775807, False
                )
            )
    ...
    def fieldTypeConversionSqlFor(
        ...
        if old_type_desc.base == "string":
            ...
            # Case: string -> number
            if new_type_desc.base in ["integer", "bigint", "numeric", "double"]:
    ...
    def isTimeoutError(self, cond):
        ...
        return isinstance(cond.orig, psycopg2.errors.QueryCanceled)  # pylint: disable=no-member
    ...
    def acquireVersionStampLock(self, exclusive=False):
        ...
        Acquiring lock is exclusive mode has "lower priority" as to not block shared locks from being obtained while waiting for the exclusive mode"""

        self.progress(3, "Acquiring version stamp lock:", "exclusive=", exclusive)
        if exclusive:
            while not self.execute("SELECT pg_try_advisory_xact_lock(1)").fetchone()[0]:
                self.progress(9, "Version stamp lock not available. Trying again in 1 sec")
                time.sleep(1)
            # lock acquired
        else:
            self.execute("SELECT pg_advisory_xact_lock_shared(1)")
    ...
    def getUpdatabilityClause(self):
        ...
        """ENH: updatability clause has support for READ ONLY or UPDATE, and an optional column name list
        SQL-92 ::=  FOR { READ ONLY | UPDATE [ OF <column name list>  ] }"""
    ...
    def loadFeaturesFrom(
        ...
        if key_field_sql_type is None:
            key_field_sql_type = self.sqlTypeFor(
                key_field_name, table_desc.columns[key_field_name].type_desc
            )
        ...
        try:
            ...
            # Incoming records include the key field
            if key_field_name in tmp_col_names:
            ...
            # Incoming records don't include the key field
            else:
                n_updated = 0

                # Insert new records
                sql = "INSERT INTO {ftr_tab} ({col_names}) SELECT {col_vals} FROM {tmp_tab}".format(
                    **sql_params
                )
