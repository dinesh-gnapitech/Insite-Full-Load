...
class MywDbDriver:
    ...
    def __init__(self, session, progress=MywProgressHandler()):
        ...
        self.language_parser = MywMultiLanguageParser()
    ...
    def dataTypeFor(self, sqa_type):
        ...
        if isinstance(sqa_type, BigInteger):
            return "bigint"
        ...
        if sqa_type_str.startswith("geometry(GEOMETRY"):
            return "geometry"
        if sqa_type_str.startswith("geography(GEOMETRY"):
            return "geometry"
    ...
    def sqlDefaultFor(self, column_desc, quote_string_values=True):
        ...
        if generator != None:
            if base_type in ("integer", "bigint") and (generator == "sequence"):
                return None
            elif (base_type == "timestamp") and (generator == "system_now"):
                return "CURRENT_TIMESTAMP"
            elif (base_type == "timestamp") and (generator == "now_utc"):
                return "CURRENT_TIMESTAMP"
        ...
        if default_value != None:
            ...
            elif type_desc.base in ["integer", "bigint", "double", "numeric"]:
    ...
    def prepareForInsert(self, feature_type, rec):
        """
        Called before feature record REC is inserted
        """
    ...
    def featureTriggerSqls(self, feature_schema, feature_rec, trigger_type):
        ...
        geom_indexed = feature_rec.geom_indexed != False
    ...
    def featureTriggerGeomIndexSql(self, feature_schema, feature_rec, trigger_type, filter_ctrl):
        ...
        geom_field_info = self.geomFieldInfoFor(feature_type)

        # Delete existing index records
        if geom_field_info and trigger_type in ["update", "delete"]:
        ...
        if trigger_type in ["insert", "update"]:
            for geom_field_name, world_field_name in list(geom_field_info.items()):
                for world_type, geom_type in self.geom_index_tables():
    ...
    def searchStringFieldExprs(
        ...
        enum_fields = self.enumFieldInfoFor(feature_type, lang)
        ...
        extra_values_expr = self.searchStringExtraValuesExpr(
            feature_rec, search_rule_recs, enum_fields, extra_values_len, trigger, lang
        )
    ...
    def searchStringExtraValuesExpr(
        self, feature_rec, search_rule_recs, enum_fields, max_len, for_trigger, lang
    ):
        """
        ...
        external_name = self.language_parser.parse(feature_rec.external_name, "", lang)
        extra_values = "'" + self.sqlEscape(external_name.lower()) + "'"
        ...
        for rec in search_rule_recs:
            ...
            extra_values += (
                " || '|' || "
                + "lower(''|| "
                + self.convertExpressionToSql(
                    feature_rec, rec.search_val_expr, enum_fields, trigger=for_trigger, lang=lang
                )
                + " )"
            )
    ...
    def enumFieldInfoFor(self, feature_type, lang):
        """
        Obtains the enumerators info used by FEATURE_TYPE

        Returns enumerator value records, keyed on feature's field name"""
        MywDDField = self.rawModelFor("myw", "dd_field")
        ...
        for field in field_recs:
            ...
            if not enum_name in enums:
                enum_value_recs = (
                    ...
                    .filter((MywDDEnumValue.enum_name == enum_name))
                    .all()
                )
                enum_values = self._translateEnumDisplayValues(enum_value_recs, lang)

                # cache in case it's used for a different field
                enums[enum_name] = enum_values
            else:
                enum_values = enums[enum_name]

            enum_info[field.internal_name] = enum_values

        return enum_info

    def _translateEnumDisplayValues(self, enum_value_recs, lang):
        """
        Translate display values of ENUM_VALUE_RECS to LANG when they are multi-lang strings
        If display value is not a multi-lang string or it doesn't contain LANG, the original display value is used

        Returns a list of dicts with the same structure as the records
        """

        enum_values = []
        for enum_value in enum_value_recs:
            dv = enum_value.display_value
            try:
                stringsByLang = json.loads(dv)
                dv = stringsByLang.get(lang, dv)
            except:
                pass  # keep original value

            enum_values.append({"value": enum_value.value, "display_value": dv})
    ...
    def rebuildGeomIndexesSqls(self, feature_schema, feature_rec):
        ...
        for world_type, geom_type in self.geom_index_tables():
            ...
            if not feature_rec.geom_indexed:
                continue
    ...
    def convertExpressionToSql(
        self, feature_rec, expression, enum_fields, max_length=None, trigger=False, lang=None
    ):
        """
        ...
        parser = self._parserFor(feature_rec, expression, lang)
        ...
        for (el_type, value) in els:
            ...
            # Case field: Escape reserved words, handle null values, ...
            elif el_type == "field":
                sql_value = self._convertFieldExpression(value, enum_fields, trigger)
    ...
    def _parserFor(self, feature_rec, expression, lang):
        """
        Returns parser for given expression
        Pseudo fields will be replaced appropriately for LANG
        """
        ...
        convert = lambda expr, missing: self.language_parser.parse(expr, missing, lang) or ""

        name = feature_rec.feature_name

        pseudo_fields = OrderedDict()
        pseudo_fields["short_description"] = convert(feature_rec.short_description_expr, "")
        pseudo_fields["title"] = convert(feature_rec.title_expr, ".".join([name, "title"]))
        pseudo_fields["display_name"] = convert(
            feature_rec.external_name, ".".join([name, "display_name"])
        )
        pseudo_fields["external_name"] = convert(
            feature_rec.external_name, ".".join([name, "display_name"])
        )  # For pre-4.3 compatibility

        return MywExpressionParser(expression, pseudo_fields)

    def _convertFieldExpression(self, field_name, enum_fields, trigger):
        """
        Convert FIELD_NAME return the SQL to obtain that field's value

        Does display_value mapping for enumerator fields"""
        sql_value = self.quotedColumnName(field_name)
        if trigger:
            sql_value = self.trigger_new + "." + sql_value

        enum_values = enum_fields.get(field_name, None)
        # if field is enumerated, apply mapping to display_value
        # if all enumerator values have the display_value the same as value, there is no need to do the mapping
        if enum_values and self._hasDisplayValues(enum_values):

            exprs = [
                "WHEN '"
                + ev["value"]
                + "' THEN '"
                + self.sqlEscape(ev["display_value"], True)
                + "'"
                for ev in enum_values
            ]
            mapping = " ".join(exprs)
            sql_value = "CASE " + sql_value + " " + mapping + " ELSE " + sql_value + " END"
    ...
    def _hasDisplayValues(self, enum_values):
        """
        Returns True if ENUM_VALUES has at least one element where display_value is different from value
        """
        areAllSame = all((ev["value"] == ev["display_value"]) for ev in enum_values)
        return not areAllSame
    ...
    def getUpdatabilityClause(self):
        """
        Returns sql FOR UPDATE clause, that sets updatability during select
        """
        raise Exception("getUpdatabilityClause not implemented")
    ...
    def isTimeoutError(self, cond):
        """
        Returns true if given COND is a timeout error
    ...
    def versionStamp(self, component, update_lock=False):
        """
        The value of the version stamp for COMPONENT (which must exist)
        update_lock will add a ROW level lock to the select, cleared when the transation is commited or rolled back.
        """
        # ENH: Use rawModel?
        ...
        sql = "SELECT version FROM {} WHERE component='{}'{}".format(
            self.dbNameFor("myw", "version_stamp", True),
            component,
            self.getUpdatabilityClause() if update_lock else "",
        )
    ...
    def sqlForTimestamp(self, timestamp):
        """
        Returns a value representing a timestamp that is appropriate for SqlAlchemy
        For most dialects, it can be the timestamp object directly
        """
        return timestamp
