...
class MywOidcAuthEngine(MywAuthEngine):
    ...
    def authenticate(self, request):
        ...
        Returns dict with keys:
         ...
         'auth_engine_metadata'   Metadata for auth engine
         'auth_redirect_info' Redirect parameters"""
        ...
        return {
            ...
            "roles": roles,
            "auth_redirect_info": {
                "redirect_to": session_info.get("redirect_to", "index"),
    ...
    def singleSignOn(self, request):
        ...
        try:
            ...
            # Store redirect_to and params in the oidcrp session to be loaded after successful authentication
            state_interface = (
                self.rp_handler.get_client_from_session_key(init_data["state"])
                .client_get("service_context")
                .state
            )

            state_interface.store_item(redirect_to, "redirect_to", init_data["state"])
            state_interface.store_item(params, "params", init_data["state"])
    ...
    def _get_redirect_uri(
        ...
        Parameters:
        base_redirect_uri: The base URL that will be used for the redirect.
        origin_client: The client origin.
        redirect_to: This parameter is deprecated.
        params: This parameter is deprecated.

        Returns:
        The modified redirect URL.
        """

        if origin_client == "anywhere":
            return f"{base_redirect_uri}/anywhere/myw_oidc_auth_engine"

        # Keep the default redirect_to as index. This will allow current configured OIDC providers to continue to work.
        # ENH: Remove redirect_to=index" from the params, as it is no longer needed.
        return f"{base_redirect_uri}?redirect_to=index"
