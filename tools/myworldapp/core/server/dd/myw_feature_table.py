...
class MywFeatureTable:
    ...
    def geomFilter(self, geom, default=False, include_zero_linestrings=False):
        """
        ...
        include_zero_linestrings will add an extra element to the predicate (which slows the query,
        becuase it isn't indexed), but ensures that all point-like linestrings are captured by the
        filter.
        ...
        for geom_field, descr in self.model._descriptor.geomFields().items():
            geom_filter |= self.field(geom_field).geomIntersects(
                geom, include_zero_linestrings and descr.type_desc.base == "linestring"
            )
            have_geoms = True
