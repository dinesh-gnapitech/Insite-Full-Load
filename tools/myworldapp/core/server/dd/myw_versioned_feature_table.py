...
class MywVersionedFeatureTable(MywFeatureTable):
    ...
    def __init__(self, view, feature_type, model, base_model, delta_model, delta, schema="data"):
        ...
        # when change_types is set, only records modified in the delta with these change types are returned
        self.change_types = None
    ...
    def changes(self, *types):
        """
        Records modified in delta with change types TYPES ('insert', 'update' or 'delete')
        """

        clone = self.clone()
        clone.change_types = types
        return clone
    ...
    def _delta_filtered_recs(self, offset=None, limit=None, order_by_info=[], batched=True):
        """
        Returns SQLAlchemy query yielding delta records of self
        """

        recs = self._delta_recs

        # when merging with master records we exclude deletions
        if not self.change_types:
            recs = recs.filter(self.delta_model.myw_change_type != "delete")
    ...
    def _master_unshadowed_recs(self):
        ...
        if self.schema == "delta" or self.change_types:
            return self._recs.filter(
                False
            )  # For 'delta' schema or when filtered on change types we never want to return any master records
    ...
    def _delta_recs(self):
        ...
        query = None
        if self.schema == "data":
            query = self.session.query(self.delta_model).filter(
                self.delta_model.myw_delta == self.delta
            )
        else:
            # schema is 'delta', include all delta recs except the current delta
            query = self.session.query(self.delta_model).filter(
                self.delta_model.myw_delta != self.delta
            )

        if self.change_types:
            query = query.filter(self.delta_model.myw_change_type.in_(self.change_types))
