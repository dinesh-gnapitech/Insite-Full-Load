...
import $ from 'jquery';
import 'jquery-ui';
import { isEqual, template, groupBy, omit, clone } from 'underscore';
...
export class BulkFeatureEditor extends Control {
    ...
    async updateFeatures(featureJson, changedFields) {
        ...
        const propertiesToMergeKeys = Object.keys(propertiesToMerge);
        //start by running pre update hook
        await Promise.all(
            this.featureSet.map(async feature => {
                let geoJson = feature.asGeoJson();
                preUpdateGeoJsons.push(geoJson);
                const cloned = clone(geoJson.properties);
                geoJson = {
                    ...geoJson,
                    properties: { ...geoJson.properties, ...propertiesToMerge }
                };
                await feature.preUpdate(geoJson, this.app);
                const differents = this._findDifferenceAfterTrigger(cloned, geoJson.properties);
                const triggerChange = omit(differents, propertiesToMergeKeys);
                if (Object.keys(triggerChange).length > 0)
                    triggerChanges[feature.getUrn()] = triggerChange;
            })
        );

        //  Perform the bulk update transaction here
        await this.datasource.bulkUpdateFeatures(
            this.featureSet,
            propertiesToMerge,
            triggerChanges
        );
    ...
    /**
     * Find different properties between original properties and properties after preUpdate trigger
     * @private
     * @param {object} original        original properties object
     * @param {object} afterTrigger    properties object after preUpdate trigger
     * @returns {object}               properties that are different between the two objects
     */
    _findDifferenceAfterTrigger(original, afterTrigger) {
        const diff = {};

        // Check properties in original
        for (let key in original) {
            if (original[key] !== afterTrigger[key]) {
                diff[key] = afterTrigger[key];
            }
        }

        // Check properties in afterTrigger
        for (let key in afterTrigger) {
            if (Object.hasOwn(afterTrigger, key) && !Object.hasOwn(original, key)) {
                diff[key] = afterTrigger[key];
            }
        }
